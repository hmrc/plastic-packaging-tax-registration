...
import play.api.libs.json.{JsError, JsObject, JsString, JsSuccess, JsValue, Json, Reads, Writes, __}

import scala.util.{Failure, Success, Try}
sealed trait SchemaError

object SchemaError {
  case class NestedSchemaError(schemaPath: String, keyword: String, instancePath: String, errors: Map[String, Seq[SchemaError]]) extends SchemaError
  case class SimpleSchemaError(message: String) extends SchemaError

  private def fromJson(json: JsValue): SchemaError = { json match {
      case msg: JsString => SimpleSchemaError(msg.as[String])
      case json: JsObject => NestedSchemaError(
  ...
  private def toJson(se: SchemaError): JsValue ={
    se match {
      ...
      case NestedSchemaError(sp, k, ip, e) => Json.obj(
        "schemaPath" -> sp,
        "keyword" -> k,
        "instancePath" -> ip,
