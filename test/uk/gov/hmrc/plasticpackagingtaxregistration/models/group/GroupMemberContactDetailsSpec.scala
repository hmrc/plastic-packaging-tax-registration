...
class GroupMemberContactDetailsSpec extends AnyWordSpec with Matchers with SubscriptionTestData {
  ...
  "GroupMemberContactDetails" should {
    "build successfully" when {
      "there is no middle name " in {
        val individualDetails         = groupPartnershipDetailsMember.individualDetails
        val groupMemberContactDetails = GroupMemberContactDetails(groupPartnershipDetailsMember)
        groupMemberContactDetails.firstName mustBe s"${individualDetails.firstName}"
        groupMemberContactDetails.lastName mustBe s"${individualDetails.lastName}"
        groupMemberContactDetails.address mustBe Some(
          PPTAddress(groupPartnershipDetailsMember.addressDetails)
        )
        groupMemberContactDetails.phoneNumber mustBe Some(
          groupPartnershipDetailsMember.contactDetails.telephone
        )
        groupMemberContactDetails.email mustBe Some(
          groupPartnershipDetailsMember.contactDetails.email
        )
      }
    }
  }
